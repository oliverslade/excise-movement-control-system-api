...
case class IE871Message(
  ...
  override def toXml: NodeSeq = {
    scalaxb.toXML[IE871Type](obj, key.getOrElse(messageType), scope)
  }
...
object IE871Message {
  def apply(message: DataRecord[MessagesOption], scope: NamespaceBinding): IE871Message = {
    IE871Message(message.as[IE871Type], message.key, scope, ShortageOrExcess)
  }

  def createFromXml(xml: NodeSeq): IE871Message = {
    val ie871: IE871Type = scalaxb.fromXML[IE871Type](xml)
    IE871Message(ie871, Some(ie871.productPrefix), TopScope, ShortageOrExcess)
  }
}